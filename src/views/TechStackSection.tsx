    ...
    >
      <div className="grid grid-cols-1 gap-x-10 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 border-t pt-10">
        {techStack.categories.map((category) => (
          <div
            key={category.id}
            className="space-y-4 sm:border-r last:border-r-0 pr-4"
          >
            <h3 className="flex items-center gap-2 text-xs text-wide-tracking text-muted">
              <span
                aria-hidden="true"
                className="font-mono normal-case text-muted"
              >
                {"<>"}
              </span>
              {category.title}
