  ...
  return (
    <UISection id="about">
      ...
      <p className="uppercase mt-3 before:inline-block before:h-1 before:w-14 before:bg-black before:content-[''] before:mr-6">
        <span className="sm:text-lg">Front-End Engineer</span>
        <span className="text-xs sm:text-sm"> & Full-Stack Developer</span>
      </p>
      <p className="mt-8 mb-16 border-y py-4 px-1 text-xs md:text-sm text-muted text-wide-tracking text-bold flex gap-x-4 gap-y-2 sm:gap-x-5 2xl:justify-between flex-wrap">
      ...
      <UISplitColumns
        aside={<h2 className="text-wide-tracking font-bold">{about.title}</h2>}
      >
        {paragraphs.map((paragraph) => (
          <p key={paragraph} className="mb-4 last:mb-0 max-w-screen-sm">
            {paragraph}
          </p>
        ))}
      </UISplitColumns>
