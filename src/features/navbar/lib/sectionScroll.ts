 ...
 * using a line a quarter of the way down the viewport as the reference: the
 * active section is the one whose top has scrolled past that line but is
 * closest to it. So a section is picked up once its top reaches the upper
 * quarter of the screen, not only once it nears the top. Returns `null`
 * before any section has reached it yet (e.g. while still reading the intro).
 */
