...
import {
  ...
  getProjectBySlug,
  getProjectIndex,
} from "@/features/projects/lib/projectList";
import { subscribeToProjectOpen } from "@/features/projects/lib/projectOpenEvent";
import {
  getProjectPath,
  getProjectSlugFromPath,
  getProjectsSectionPath,
} from "@/features/projects/lib/projectPaths";
import type { Locale } from "@/i18n/config";
import { pushPathname, replacePathname } from "@/lib/history";
...
export function useActiveProject(
  ...
  useEffect(
    () =>
      subscribeToProjectOpen((projectId) => {
  ...
  function close() {
    setOpen(false);
    pushPathname(getProjectsSectionPath(locale));
  }
  ...
  const goTo = useCallback(
    (direction: "previous" | "next") => {
      ...
      replacePathname(getProjectPath(locale, nextProject));
    },
